...
Finally, let's write code to grab all of the pages. To speed things up, we will
use only the first 10 stories but the code can be easily changed to include all
of them.

Note that here we need to collect two different tables, one for metadata and 
one for stories. The unique id created above will help if we later want to put
